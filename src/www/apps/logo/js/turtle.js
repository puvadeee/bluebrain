...
function Turtle (canvas, turtle) {
    ...
    if (canvas && canvas.getContext) {
		...
		this.turtleSprite = loadSprite("images/turtlespace-sprite.png");
...
Turtle.prototype.update = function() {
    if (this.visible && this.x >=0 && this.y >= 0 && this.x <= this.max_x && this.y <= this.max_y) {
        this.turtle.style.left = parseInt(this._left + this.x - 32) + "px";
        this.turtle.style.top = parseInt(this._top + this.y - 32) + "px";

        this.spriteCtx.clearRect(0, 0, 64,64);
        drawSprite(this.spriteCtx, this.turtleSprite, 32 , 32, ( (90+this.angle) * Math.PI)/180, 0.25);
    } else {
...
Turtle.prototype.forward = function (d) {
    ...
    this.crawl(d);
    if (d > 0)
        sendTurtleCommand("FORWARD", d)
    else
        sendTurtleCommand("BACKWARD", Math.abs(d))
}
...
Turtle.prototype.right = function(angle) {
    ...
    if (angle>0)
        sendTurtleCommand("RIGHT", angle)
    else
        sendTurtleCommand("LEFT", Math.abs(angle))
}
