 ...
 */
// modified from:  https://github.com/jgautier/firmata

/**
 * Module Dependencies
 */
/*
 var util = require("util"),                         // see inherits just below
 Emitter = require("events").EventEmitter,         // replacement from: https://github.com/caolan/events/blob/master/events.js
...
define(['encoder7bit', 'onewireutils', 'serialport', 'events', 'BlueBrain'], function (Encoder7Bit, OneWireUtils, SerialPort, events, cannybots) {
    

    
    var Emitter = events.EventEmitter;
    // instead of 'util' module
    ...
    // from : https://github.com/react-bootstrap/react-bootstrap/blob/master/src/utils/Object.assign.js
    ...
    /*
     ...
     cannybots.debug("SPEX: " + err);
     SerialPort = null;
     }
     */
    
    if (SerialPort == null) {
        cannybots.debug("It looks like serialport didn't compile properly. This is a common problem and its fix is well documented here https://github.com/voodootikigod/node-serialport#to-install");
    ...
    var Board = function(port, options, callback) {
        ...
        this.sp.on("data", function(dataBuffer) {
            var byt, cmd;
            var data = new Uint8Array(dataBuffer);

            //cannybots.debug("Firmata, on data[0]: '" + typeof data + "'");
            //cannybots.debug("Firmata, on data[0]: '" + typeof data[0] + "'");
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                byt = data[i];
                //cannybots.debug("Firmata, data["+i+"]= " + byt);
                    ...
                    if (this.currentBuffer[0] === START_SYSEX &&
                        ...
                        this.currentBuffer[this.currentBuffer.length - 1] === END_SYSEX) {
                            ...
                            if (cmd !== REPORT_VERSION && cmd !== ANALOG_MESSAGE && cmd !== DIGITAL_MESSAGE) {
                                // cannybots.debug("OUT OF SYNC - CMD: "+cmd);
                                // Clean buffer
                                this.currentBuffer.length = 0;
                            }
                        }
        ...
        //  SEND:  249             =    REPORT_VERSION          0xF9
        
        //  SEND:  [240,121,247]   =    START_SYSEX             0xF0
        //                              REPORT_FIRMWARE         0x79 // report name and version of the firmware
        //                              END_SYSEX               0xF7
        
        //  RECV   [0xf9, 0x2, 0x3]=    REPORT_VERSION          0xF9            2.3
        ...
        this.once("reportversion", function() {
            cannybots.debug ("ONCE:  reportversion");
            clearTimeout(this.reportVersionTimeoutId);
            this.versionReceived = true;
            
            cannybots.debug ("SENDING:  queryfirmware");

            this.once("queryfirmware", function() {
                cannybots.debug ("ONCE:  queryfirmware");
                
                if (settings.samplingInterval) {
                    cannybots.debug("settings.samplingInterval");
    ...
    Board.prototype.digitalWrite = function(pin, value) {
        ...
        var portValue = 0;
        this.pins[pin].value = value;
        for (var i = 0; i < 7; i++) { // was 8, probably assumed arduino, TODO: GPIO
